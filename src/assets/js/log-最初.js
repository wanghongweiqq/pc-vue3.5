...
const formatDateToString = () => {
  ...
  const result = reg.replace(/([hmsl])+/g, (match, key) =>
    f[key].toString().padStart(match.length, '0'),
  )
  return result
}
...
const log = function () {
  ...
  if (args.length > 0 && colorsMap[args[0]]) {
    ...
    console.info(colorKey)
    console.info(colorValue)
    args.shift() // 删除标识颜色、样式的参数
    // if (IS_HARMONY) { // 给鸿蒙添加输出时间，iOS和android默认已有时间输出，精确到毫秒级
    ...
    // iOS: HH:MM:SS  鸿蒙：16:44:02 GMT+0800 (CST)，怎么设置输出格式都不管用，貌似鸿蒙不支持
