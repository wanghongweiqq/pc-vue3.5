...
function render() {
  return createVNode("div", null, [
    ...
    <p>编译器分析模板，给动态节点打上 PatchFlag 标记，运行时 diff 直接根据 flag 判断哪些属性可能变化，精准更新，跳过其余属性的比较。</p>
    <pre class="code-block">
...
&lt;div&gt;             ← Block 根节点
  ...
  &lt;i :class="b"&gt;&lt;/i&gt;     ← 进 dynamicChildren（CLASS）
&lt;/div&gt;

diff 时：只遍历 dynamicChildren = [span, i]，静态 p 直接跳过</pre>
...
记录可复用节点在新序列中的位置数组：[1, 2, 0]（对应 c d e）</pre>
...
最终：1 次 unmount（f）+ 1 次 mount（h）+ 1 次 move（e），共 3 次 DOM 操作</pre>
