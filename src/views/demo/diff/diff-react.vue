<template>
  <div class="content">
    ...
    <p>React Fiber 是 React 16（2017）引入的全新协调器（Reconciler），用来替代旧版的 Stack Reconciler。核心目标是把原来<em>同步、不可中断</em>的递归渲染改成<em>可中断、可恢复、带优先级</em>的增量渲染。</p>

    <h3>一、为什么需要 Fiber</h3>
    <p>旧版 React 用 JS 调用<em>栈递归遍历组件树</em>，一旦开始就必须跑完。组件树很深时，长时间占用主线程，导致用户点击 / 输入 / 动画无响应（掉帧）。</p>
...
// 通过 child → sibling → return 指针可中断遍历、记住进度</pre>
