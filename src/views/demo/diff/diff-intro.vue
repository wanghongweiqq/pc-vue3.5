...
n × n = O(n²) 对，再对每对节点计算子节点最优映射 × O(n) = O(n³)
` }}</pre>
    <table class="table">
      <tbody>
        <tr>
    ...
    <p>节点类型（标签名 / 组件名）不同，直接销毁旧子树、创建新子树，不做任何子节点比较。</p>
    <table class="table">
      <tbody>
        <tr>
          <th>旧树</th>
    ...
    <h4>② 同层比较<small>-只比较同一层级的节点，不跨层级移动</small></h4>
    ...
    <h4>③ key 标记身份<small>-用 key 帮助算法识别可复用节点，避免错位</small></h4>
