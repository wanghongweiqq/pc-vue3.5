    ...
    getList () {
      ajax.getList(this.query).then((res) => {
        if (res.success) {
          ...
          // 如果 const list = [ …… ]为静态数据，后面this.queryList = list ，每调用一次getList这个方法，就相关于改变了引用地址，所以会触发响应式更新。
          ...
          this.queryList = list
          setTimeout(() => {
            list[0].customerName = '2' // 不更新
            // this.queryList = list // 不更新
            // this.queryList[0].customerName = '2' // 更新
            // this.queryList.sort(sortFun) 
          }, 2000)
          // 更新，直接操作响应式数组
          // this.queryList.sort(sortFun) 
        } else {
