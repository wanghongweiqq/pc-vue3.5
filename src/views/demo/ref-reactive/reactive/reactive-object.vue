...
const editMark = () => {
  ...
  // mark = { x: Date.now() } // 丢失响应式：变量mark指向一个新的内存地址
    ...
    <h3>watch 监听说明</h3>
    <p>reactive 声明的数组/对象，只有在<em>不改变根数据的引用</em>的情况下才能被监听到，但此时新旧值相同（引用未变），无法做新旧差异比较（如果想做新旧比较，可以watch具体的属性，如() => reactiveObject.name）。改变根引用时，watch 监听的还是最初的引用地址，那个地址的数据没有变化，所以不会触发。</p>
    <p>reactive 声明的响应式数据，默认配置deep=true，此时非根层级的引用变化和内部属性变化都能监听到，deep为false时，只能监听到引用变化。ref 声明的响应式数据，需要配置deep:true，默认deep=false。</p>
