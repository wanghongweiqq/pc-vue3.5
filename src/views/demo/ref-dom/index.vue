...
const toggleAndFocus = async () => {
  ...
  inputRef.value?.focus() //inputRef为input的dom
        ...
        <el-tab-pane
          ...
          name="dom-ref"
        >
          <!-- ref="domRefComp" 用于拿到子组件实例，调用其 defineExpose 暴露的 runTask1 -->
          <RefDomRef ref="domRefComp" />
        </el-tab-pane>
...
import RefReact from './ref-react'
import RefUseTemplateRef from './ref-useTemplateRef'
import RefDomRef from './ref-dom-ref'

const activeTab = ref('options')

// 指向 RefDomRef 子组件实例，用于调用其 defineExpose 暴露的 runTask1
const domRefComp = ref(null)

// @tab-change 事件在每次切换 Tab 后触发，参数为新激活 Tab 的 name。
// nextTick 确保 el-tab-pane 从 display:none 切换为可见后再调用 runTask1，
// 此时 innerText 才能正确触发 reflow，感知 CSS 可见性，与 textContent 产生明显差异。
const onTabChange = async (name) => {
  if (name === 'dom-ref') {
    await nextTick()
    domRefComp.value?.runTask1()
  }
}
