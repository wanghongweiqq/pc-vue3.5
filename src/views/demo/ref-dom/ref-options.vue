<template>
  <div class="content">
    ...
    <h4>其他方法</h4>
    <p>通过子组件监听父组件属性的变化也可让子组件直接操作相关方法，但有个问题不能忽略，就是子组件是watch这个props，这个属性要有改变才能监听到。比如示例的子组件input获取焦点focus，父组件第一次改变其属性isFocus由false->true，可以被子组件watch到，但后续什么时机改变这个值为false，要做特殊的逻辑处理，否则下次点击可能就不会监听到属性isFocus变化，就不会触发子组件的相关dom方法。<em>所以这种形式要慎用</em>。</p>
    <p>
      <el-button
        size="small"
        type="primary"
